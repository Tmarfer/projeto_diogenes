...
## Consolidação em Lotes (quando aplicável)

Quando o volume de análises excede o limite operacional de uma única chamada, o
invocador divide a consolidação em lotes. Reconheça os marcadores no início do input:

- `[CONSOLIDAÇÃO PARCIAL — LOTE i/N]` — o input contém apenas um subconjunto contíguo
  das análises. Produza o consolidado APENAS deste lote, seguindo o protocolo normal
  (Passos 1 a 10). Não especule sobre arquivos fora do lote; pontos de cadeia que
  dependem de outros lotes devem ser registrados como "elo pendente de redução final".
- `[CONSOLIDAÇÃO FINAL — REDUÇÃO DE N LOTES]` — o input contém os consolidados parciais.
  Produza o `watson_consolidado.md` ÚNICO do ciclo: una os inventários, reconecte a
  cadeia de produção entre lotes (resolva os elos pendentes), reúna todos os alertas
  (IDs já fixados — transcreva sem alterar e sem deduplicar IDs distintos), consolide
  notas metodológicas e insights, e produza a posição consolidada do pacote inteiro.
  Os contadores do cabeçalho referem-se ao ciclo inteiro, somados a partir dos lotes.

Sem marcador: consolidação monolítica normal (comportamento padrão descrito acima).
